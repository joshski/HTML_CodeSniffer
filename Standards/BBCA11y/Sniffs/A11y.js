...
var HTMLCS_BBCA11y_Sniffs_A11y = {
    ...
    {
      function writeResultError(result, error) {
        var id = result.standard.replace(/(\w)(\w*)/g, function(g0, g1, g2) {
          return g1.toUpperCase() + g2.toLowerCase();
        }).replace(/\W/g, '');
        HTMLCS.addMessage(HTMLCS.ERROR,
            elementFromError(error) || top,
            messageFromError(error),
            id);
      }

      function elementFromError(error) {
        for (var e = 0; e < error.length; e++) {
          if (error[e][0] == '/') {
            var css = xPathToCss(error[e]);
      ...
      function messageFromError(error) {
        return error.filter(function(messageSection) {
          return messageSection[0] != '/';
        }).join(" ").replace(/:$/, '.');
      }

      function xPathToCss(xpath) {
        // a hack that will probs break, move into bbc-a11y
        var css = xpath.replace(/\//g, ' ').replace(/\[(\d+)\]/g, function(g0, g1) {
          return ':nth-of-type(' + g1 + ')';
        });
        return css;
      }

      var results = window.a11y.validate().results;
      for (var i = 0; i < results.length; i++) {
        for (var j = 0; j < results[i].errors.length; j++) {
          writeResultError(results[i], results[i].errors[j]);
        }
      }
    }
};
